...
- The batch loop in `main.java` for generating figure data is commented out and may need adjustments to work with the current setup.
...
| File | Purpose |
|------|---------|
| `main.java` | Main entry point. Runs Greedy 1, Greedy 2, and P-MARL sequentially. Also prints the cost/prize arrays needed as input for ILP in IBM CPLEX. |
| `TableData.java` | Batch runner used to generate the data behind Figs 3, 4, and 5. Loops over multiple cities and budgets and prints prize/distance arrays. |
...
1. Run `main.java` with `fileName = "Capital_Cities10.txt"`. This prints the cost and prize arrays and runs P-MARL and both greedy algorithms.
2. Copy the printed `Cost` and `p` arrays into IBM CPLEX Optimization Studio to get the ILP optimal solution.
3. The commented-out batch loop in `main.java` (lines ~80–117) automates running across multiple cities and budgets — **uncomment this block** to generate the full dataset. It iterates over 4 budgets × 10 cities, collecting prize and distance output for each run.
